...
for j in range(0,30):
    sp_seed = rList[3*j]
    fs_seed = rList[3*j+1]
    os.system('mkdir -p /home/mohit/updated-dBp/symfiles/no-fp/256/'+str(sp_seed)+'/no-fs/')
    os.system('mkdir -p /home/mohit/updated-dBp/symfiles/no-fp/256/'+str(sp_seed)+'/fs/'+str(fs_seed)+'/')
