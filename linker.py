...
def create_linker_script(sections, path=None):
    with open(path if path else config.link_script, 'w') as f, open(os.path.join(config.ld_dir, 'ld1'), 'r') as ld1, open(os.path.join(config.ld_dir, 'ld2'), 'r') as ld2:
